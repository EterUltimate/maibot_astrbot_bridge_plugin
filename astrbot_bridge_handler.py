...
在 MaiBot 启动时:
1. 注册 WS 自定义消息处理器，接收来自 AstrBot 的 tool_sync 和 tool_result 消息。
2. Patch send_message 使 platform="astrbot" 的回复走 API Server。
"""
...
class AstrBotBridgeInitHandler(BaseEventHandler):
    """ON_START 事件处理器 — 注册 tool_sync/tool_result 处理器 + send_message 路由补丁"""

    event_type = EventType.ON_START
    handler_name = "astrbot_bridge_init"
    handler_description = "在启动时注册 AstrBot 工具桥接的 WS 消息处理器并修补消息路由"
    weight = 0
    ...
    async def execute(
        ...
        try:
            ...
            from .tool_bridge import (
                _handle_tool_sync_ws,
                _handle_tool_result_ws,
            )
